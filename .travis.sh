...
apt-get update -qq && apt-get install -y -q wget sudo lsb-release gnupg git sed build-essential curl # for docker
...
catkin build --catkin-make-args run_tests -- -p1 -j1 --no-status
